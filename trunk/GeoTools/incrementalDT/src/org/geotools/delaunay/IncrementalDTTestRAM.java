...
public class IncrementalDTTestRAM extends TestCase {
	LinkedList points = new LinkedList();
	DelaunayDataStoreRAM triangles = new DelaunayDataStoreRAM();
	IncrementalDT triangulace=new IncrementalDT(triangles);
	
	public IncrementalDTTestRAM(String name){
		super(name);
	}
	...
	protected boolean delaunay(TriangleDT T, PointDT A){
		...
		double R=Math.sqrt(Math.pow((T.A.x-M),2)+Math.pow((T.A.y-N),2)); 			
		if ((R-0.0001)>(Math.sqrt(Math.pow((A.x-M),2)+Math.pow((A.y-N),2))))		
			return true;
		else{
			if (R>0){
				return false;
	...
	public static Test suite(){
		return new TestSuite(IncrementalDTTestRAM.class);
	}
