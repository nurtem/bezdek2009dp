...
//import org.geotools.delaunay.DataStoreHDD.Element;
...
public class DelaunayDataStoreRAM implements DelaunayDataStore {
	...
	private LinkedList deletedIdx = new LinkedList();										
	...
	public DelaunayDataStoreRAM() {
		dd.addField(Integer.class);
		try {
			PageStore ps = new MemoryPageStore(dd); // RTree bude v RAM mze i
													// do souboru
			rt = new RTree(ps); // konstruktor RTree
	...
	private static class Element implements Comparable { // pomocna trida pro
															// serazeni
		
		double distance; // vzdalenost od hledaneho bodu
		TriangleDT T; // index do kolekce bodu
	...
	public void insertToTree(TriangleDT T, double[] key) {
		...
		try {
			if (deletedIdx.size()>0){
				...
				rt.insert(env, data);
				triangles.set((Integer)deletedIdx.getFirst(),T);
				deletedIdx.removeFirst();
			}
			else{
				data.addValue(numberOfTriangles);
				rt.insert(env, data);
				triangles.add(numberOfTriangles, T);
				numberOfTriangles++;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void delete(double[] key) {
		env = new Envelope(new Coordinate(key[0], key[1])); // oblaka pro bod
		try {
	...
	public TriangleDT getTriangle(double []key){
		...
		if (data != null)
			return (TriangleDT)triangles.get((Integer)data.getValue(0));
		else
			return null;
	}
	...
	public TriangleDT getTriangle(int index){
		if (deletedIdx.contains(index)){
			//System.out.println("VRACI NULL");
			return null;
		}
		return (TriangleDT)triangles.get(index);
	}
	...
	public TriangleDT[] countN_Nearest(Coordinate P, Collection res, int number) {
		...
		while (it.hasNext()) { // count distance
			...
			TriangleDT T = (TriangleDT)triangles.get((Integer)data.getValue(0));
			Coordinate N = new Coordinate(T.key[0],T.key[1]);
			double distance = N.distance(P);
			Element element = new Element(distance, T);
			vector.add(element); // vector for saving founded trianglest
		}
		Collections.sort(vector); // sorting vector
		it = vector.iterator();
		TriangleDT[] sortedArray = new TriangleDT[number]; // creating sorted array
	...
	public Collection getIntersectEnvelope(Envelope env) {
		...
		try{
			...
			while (iter.hasNext()){
				data = (Data)iter.next();
				list.add(triangles.get((Integer)data.getValue(0)));
			}
		}
		catch (Exception e){
			e.printStackTrace();
	...
	public TriangleDT preorderTest(Coordinate P) {
		//System.out.println("Preorfer");
		for (int i = 0; i<numberOfTriangles;i++){
			TriangleDT T = (TriangleDT)triangles.get(i);
			if (T.contains(new PointDT(P)))
				return T;
		}
		return null;
	}
	...
	public void createShapefile(String path, String File, String EPSG){
		   try{	
		    	...
		    	if  (EPSG == ""){
		    		...
		    		geom = AttributeTypeFactory.newAttributeType("the_geom",
		    			LinearRing.class, true, 0, null, null);//CRS.decode(EPSG));    
		    	}
		    	...
		    	FeatureStore featureStore = (FeatureStore)(datastore.getFeatureSource("Triangle"));
		    	...
		    	FeatureWriter aWriter = datastore.getFeatureWriter("Triangle",
 		    	...
 		    	for (int i =0; i<numberOfTriangles ; i++){
		    		...
		    		if (T!=null){
		    			...
		    			aNewFeature.setAttribute(1, (Object)i);
		    		}	
	...
	/***********************************************************
	 * The method get collection of all triangles
	 * @return collection of all triangles
	 */
	public Collection getCollection(){
		LinkedList res = new LinkedList();
		for (int i=0; i<numberOfTriangles; i++){
			res.add(triangles.get(i));
		}
		return res;
	}

	/*****************************************************************
	 * The method get number of triangles
	...
	/******************************************************************
	 * The method set number of triangles
